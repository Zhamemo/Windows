...
plt.ylabel('|tx|/mm', fontsize=10)
plt.ylim(0, 1200)
for x1, y1_yaw in enumerate(y1_yaw):
    plt.text(x1, y1_yaw, y1_yaw, ha='center', color='k', fontsize=6)
for x2, y2_yaw in enumerate(y2_yaw):
    plt.text(x2 + 0.4, y2_yaw, y2_yaw, ha='center', color='k', fontsize=6)
...
plt.legend(loc='upper left')
plt.xticks(x + bar_width / 2, tick_label)
plt.ylabel('|ty|/mm', fontsize=10)
# plt.ylim(0, 1.7)
for x1, y1_pitch in enumerate(y1_pitch):
    plt.text(x1, y1_pitch, y1_pitch, ha='center', color='k', fontsize=6)
for x2, y2_pitch in enumerate(y2_pitch):
    plt.text(x2 + 0.4, y2_pitch, y2_pitch, ha='center', color='k', fontsize=6)
...
plt.legend(loc='upper left')
plt.xticks(x + bar_width / 2, tick_label)
plt.ylabel('|tz|/mm', fontsize=10)
for x1, y1_roll in enumerate(y1_roll):
    plt.text(x1, y1_roll, y1_roll, ha='center', color='k', fontsize=6)
for x2, y2_roll in enumerate(y2_roll):
    plt.text(x2 + 0.4, y2_roll, y2_roll, ha='center', color='k', fontsize=6)
