...
% 平面参数
% X_plane = inv(A_plane'*A_plane)*(A_plane')*l
...
fprintf(['平面拟合中误差：',num2str(error_plane),' mm\n']);
% 球面拟合结果
fprintf('####################    球面拟合     #####################\n');
...
fprintf(['拟合球面半径 R; ',num2str(R),' mm\n']);
fprintf(['拟合球面中误差：',num2str(error_sphere),' mm\n']);
% 空间圆
fprintf('#####################    空间圆     ######################\n');
fprintf('圆心坐标; ');
...
fprintf(['空间圆半径 R; ',num2str(r),' mm\n']);
fprintf('各观测点圆度：\n')
for i = 1:number_points
    fprintf([num2str(i),':\t ',num2str(v(i)),' mm\n']);
end
fprintf(['空间圆拟合中误差：',num2str(error_circle),' mm\n']);
...
% title('空间圆拟合');
axis equal,xlabel('X/mm'),ylabel('Y/mm'),zlabel('H/mm'),grid;
